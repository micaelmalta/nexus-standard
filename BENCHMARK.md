...
The NXS WAL (`rust/src/wal.rs`) enables streaming span ingestion without rewriting the tail-index on every append. Each span is encoded as a raw NXSO record and appended to a `.nxsw` file; when the segment reaches its threshold the WAL is *sealed* — replayed into a fully-indexed `.nxb` file with a single tail-index pass.
...
- **append** — encode one span via `NxsWriter` and **write NXSO bytes to a tmpfs file**; no tail-index update. Includes real I/O cost.
...
| **NXS WASM**      | `encode_span` in WASM (C, no libc); JS fills input struct, WASM writes NXSO bytes  | ~2,650k spans/s | ~375 ns | 1.16 MB / 10k | 46%     |
...
| ----------------------- | ----------------------------- | ------- |
| NXS WAL raw (NXSO only) | 110.6 B                       | **44%** |
